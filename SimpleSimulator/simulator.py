...
O_check = { "11010":"hlt","01111":"jmp",  "11100":"jlt",  "11101":"jgt",  "11111":"je", "00100":"ld" ,  "00101":"st","00011":"mov",  "00111":"div",  "01101":"not", "01110" :"cmp","00010":"mov", "01000":"rs",  "01001":"ls","00000":"add",  "00001":"sub", "00110":"mul" , "01010":"xor","01011":"or" , "01100":"and" }    # Labelling operations in A
final = {}
...
def sub(r1, r2):
    sum = str('{0:016b}'.format(int(r1,2) - int(r2,2)))
    return sum


def mul(r1, r2):
    sum = '{0:016b}'.format(int(r1,2)*int(r2,2))
    return sum
def div(r1, r2):
...
while(j<i):
    ...
    if c in O_A :            
        ...
        if c=='sub':
            R[Reg[k[j][7:10]]] = sub(R[Reg[k[j][10:13]]],R[Reg[k[j][13:16]]])
        if c=='mul':
            R[Reg[k[j][7:10]]] = mul(R[Reg[k[j][10:13]]],R[Reg[k[j][13:16]]])
            Flags['V']
        if c=='xor':
        ...
        final[w] = str('{0:07b}'.format(j)) + " " + R["R0"]+ " " +R["R1"]+" " +R["R2"]+ " " +R["R3"]+" " +R["R4"]+" " +R["R5"] + " " +R["R6"] + ' ' + '0'*12 + str(Flags['V']) + str(Flags['L']) + str(Flags["G"]) + str(Flags['E'])
        break        
    final[w] = str('{0:07b}'.format(j)) + " " + R["R0"]+ " " +R["R1"]+" " +R["R2"]+ " " +R["R3"]+" " +R["R4"]+" " +R["R5"] + " " +R["R6"]+ ' ' + '0'*12 + str(Flags['V']) +str(Flags['L']) + str(Flags["G"]) + str(Flags['E'])
    if(len(pc_jmp)!=0):
        j = pc_jmp.pop()
